...
#        self._beta = np.ones(nmix) * (self._beta0 + avr_N)
        self._u = np.ones(nmix) * (self._u0)
        self._nu = np.ones(nmix) * (self._nu0)
        self._beta = np.ones(nmix) * (self._beta0)
    ...
    def _update_parameters(self,min_cv=None):
        ...
        for k in xrange(nmix):
            self._m[k] = (self._N[k]*self._xbar[k] \
                + self._beta0*self._m0) / self._beta[k]
            dx = self._xbar[k] - self._m0
            self._V[k] = self._V0 + self._C[k] + self._N[k]*self._beta0 \
